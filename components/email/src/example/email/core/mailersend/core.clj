(ns example.email.core.mailersend.core
  (:require [clj-http.client :as http]
            [clojure.tools.logging :as log]
            [rum.core :as rum]))


(defn signin-link [{:keys [to url user-exists app-name]}]
  (let [[subject action] (if user-exists
                           [(str "Sign in to " app-name) "sign in"]
                           [(str "Sign up for " app-name) "sign up"])]
    {:to      [{:email to}]
     :subject subject
     :html    (rum/render-static-markup
               [:html
                [:body
                 [:p "We received a request to " action " to " app-name
                  " using this email address. Click this link to " action ":"]
                 [:p [:a {:href   url
                          :target "_blank"} "Click here to " action "."]]
                 [:p "This link will expire in one hour. "
                  "If you did not request this link, you can ignore this email."]]])
     :text    (str "We received a request to " action " to " app-name
                   " using this email address. Click this link to " action ":\n"
                   "\n"
                   url "\n"
                   "\n"
...
(defn signin-code [{:keys [to code user-exists app-name]}]
  (let [[subject action] (if user-exists
                           [(str "Sign in to " app-name) "sign in"]
                           [(str "Sign up for " app-name) "sign up"])]
    {:to      [{:email to}]
...
(defn send-mailersend [{:keys [biff/secret mailersend/from mailersend/reply-to app-name]} form-params]
