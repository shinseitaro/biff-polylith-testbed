...
;; app-name は context map から引き取る形に変更
(defn base [{:keys [::recaptcha app-name] :as ctx} & body]
  (apply
   biff/base-html
   (-> ctx
       (merge #:base{:title app-name ;; settings/app-name
                     :lang "en-US"
                     :icon "/img/glider.png"
                     :description (str app-name " Description")
